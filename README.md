...
can override both locations. Uploaded images are stored in `uploads/`. The server
keeps one recoverable PNG for every current Gallery template in `downloads/`. It
synchronizes that folder at startup and nightly at 2:00 AM, rewriting files only when
their templates change and removing backups for deleted templates. Inventory starts
at zero with a default low-stock threshold of 100 beads per color.
