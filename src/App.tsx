...
function ArtworkTile({ template, artwork, onView, onEdit, onComplete, onExport, onRemoveArtwork, onRemove }: {
  template: Template; artwork?: Artwork; onView: () => void; onEdit: () => void; onComplete: () => void; onExport: () => void; onRemoveArtwork?: () => void; onRemove?: () => void;
}) {
...
function App() {
  ...
  const removeArtwork = async () => { if (!artworkDeleteTarget) return; setBusy(true); try { const removed = artworkDeleteTarget; await api(`/api/artworks/${removed.id}`, { method: "DELETE" }); setArtworkDeleteTarget(null); await refresh(); notify(`Removed the completed photo for “${removed.templateTitle}”.`); } catch (cause) { notify(cause instanceof Error ? cause.message : "Could not remove the completed photo.", true); } finally { setBusy(false); } };
  ...
  return <main><header className="app-header"><div className="brand"><div className="brand-mark" aria-hidden="true">{Array.from({ length: 9 }, (_, i) => <span key={i} />)}</div><div><p className="eyebrow">My collection</p><h1>拼豆Pro</h1></div></div><nav aria-label="Main navigation">{(["home", "upload", "gallery", "inventory"] as View[]).map((item) => <button key={item} className={view === item ? "active" : ""} onClick={() => { setView(item); setDraft(null); if (item === "inventory") setSelectedSeries(null); }}>{item === "inventory" && lowColors.length > 0 && <b>{lowColors.length}</b>}{NAV_LABELS[item]}</button>)}</nav></header>
    ...
    {artworkDeleteTarget && <div className="gallery-modal-backdrop" onClick={(event) => { if (event.target === event.currentTarget && !busy) setArtworkDeleteTarget(null); }}><section className="gallery-modal delete-modal" role="alertdialog" aria-modal="true" aria-label="Confirm completed photo removal"><header><div><p className="eyebrow">Remove completed photo</p><h2>Remove this uploaded image?</h2></div><button type="button" aria-label="Cancel completed photo removal" onClick={() => setArtworkDeleteTarget(null)} disabled={busy}>×</button></header><p>The photo attached to “{artworkDeleteTarget.templateTitle}” will be deleted. Its bead template will stay in your Gallery.</p><div className="gallery-modal-actions"><button type="button" className="secondary-button" onClick={() => setArtworkDeleteTarget(null)} disabled={busy}>Cancel</button><button type="button" className="primary-button delete-confirm-button" onClick={() => void removeArtwork()} disabled={busy}>{busy ? "Removing…" : "Remove photo"}</button></div></section></div>}
